...
namespace Dang.Common
{
    ...
    public class Utils
    {
        ...
        /// <returns></returns>
        public static string ReplaceMobileStartZero(string mobile)
        {
            Regex reg = new Regex(@"^0+");
            mobile = reg.Replace(mobile, "");
            return mobile;
        }
        ...
        {
            return Regex.IsMatch(strMobile, @"^1[3456789]\d{9}$");
        }
        /// <summary>
        /// 检查身份证,是否符合规范，包括长度，类型
        /// </summary>
        /// <param name="strIdcard">要判断的idcard字符串</param>
        /// <returns>判断结果</returns>
        public static bool IsValidIdCard(string strIdcard)
        {
            if (Regex.IsMatch(strIdcard, @"(^\d{15}$)|(^\d{17}(\d|X)$)"))//检查号码是否符合规范，包括长度，类型
            {
                ...
                string[] arr = new[]
                {
                    ...
                    "37","41","42","43","44","45","46","50","51","52","53","54","61","62","63","64",
                    "65","71","81","82","91"
                };
                string province = strIdcard.Substring(0, 2);//取身份证前两位,校验省份
                if (arr.Any(s => s == province))
                {
                    return true;
                }
                return false;
        ...
        /// <param name="strIdcard">要判断的idcard字符串</param>
        /// <returns>判断结果</returns>
        public static bool IsValidIdCardBirthday(string strIdcard, int beginAge, int endAge)
        {
            var len = strIdcard.Length;
            //身份证15位时，次序为省（3位）市（3位）年（2位）月（2位）日（2位）校验位（3位），皆为数字
            if (len == 15)
            {
                var re_fifteen = @"^(\d{6})(\d{2})(\d{2})(\d{2})(\d{3})$";
                var arr_data = Regex.Match(strIdcard, re_fifteen);
                var year = arr_data.Groups[2].Value;
                var month = arr_data.Groups[3].Value;
                var day = arr_data.Groups[4].Value;
                var birthday = new DateTime(Convert.ToInt32("19" + year), Convert.ToInt32(month), Convert.ToInt32(day));
                return verifyBirthday(Convert.ToInt32("19" + year), Convert.ToInt32(month), Convert.ToInt32(day), birthday, beginAge, endAge);
            }
            //身份证18位时，次序为省（3位）市（3位）年（4位）月（2位）日（2位）校验位（4位），校验位末尾可能为X
            if (len == 18)
            {
                var re_eighteen = @"^(\d{6})(\d{4})(\d{2})(\d{2})(\d{3})([0-9]|X)$";
                var arr_data = Regex.Match(strIdcard, re_eighteen);
                var year = arr_data.Groups[2].Value;
                var month = arr_data.Groups[3].Value;
                var day = arr_data.Groups[4].Value;
                var birthday = new DateTime(Convert.ToInt32(year), Convert.ToInt32(month), Convert.ToInt32(day));
                return verifyBirthday(Convert.ToInt32(year), Convert.ToInt32(month), Convert.ToInt32(day), birthday, beginAge, endAge);
            }
            return false;
        }
        ...
        /// <returns></returns>
        static bool verifyBirthday(int year, int month, int day, DateTime birthday, int beginAge, int endAge)
        {
            var now_year = DateTime.Now.Year;
            var now_day = DateTime.Now.Day;
            ...
            if (birthday.Year == year && birthday.Month == month && birthday.Day == day)
            {
                ...
                if (time > beginAge && time < endAge)
                {
                    return true;
                }
                if (time == endAge)
                {
                    time = month - now_month;
                    if (time > 0)
                    {
                        return true;
                    }
        ...
        /// <param name="strIdcard">要判断的idcard字符串</param>
        /// <returns>判断结果</returns>
        public static bool IsValidIdCardParity(string strIdcard)
        {
            //15位转18位
            strIdcard = changeFivteenToEighteen(strIdcard);
            var len = strIdcard.Length;
            if (len == 18)
            {
                var arrInt = new[] { 7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2 };
                var arrCh = new[] { '1', '0', 'X', '9', '8', '7', '6', '5', '4', '3', '2' };
                int cardTemp = 0;
                char valnum;
                for (int i = 0; i < 17; i++)
                {
                    cardTemp += Convert.ToInt32(strIdcard.Substring(i, 1)) * arrInt[i];
                }
                valnum = arrCh[cardTemp % 11];
                if (valnum.ToString() == strIdcard.Substring(17, 1))
                {
                    return true;
                }
                return false;
        ...
        /// <returns></returns>
        static string changeFivteenToEighteen(string strIdcard)
        {
            if (strIdcard.Length == 15)
            {
                ...
                var arrCh = new[] { '1', '0', 'X', '9', '8', '7', '6', '5', '4', '3', '2' };
                var cardTemp = 0;
                strIdcard = strIdcard.Substring(0, 6) + "19" + strIdcard.Substring(6, strIdcard.Length - 6);
                for (int i = 0; i < 17; i++)
                {
                    cardTemp += Convert.ToInt32(strIdcard.Substring(i, 1)) * arrInt[i];
                }
                strIdcard += arrCh[cardTemp % 11];
                return strIdcard;
            }
            return strIdcard;
        }
        ...
        /// <param name="strIdcard"></param>
        /// <returns></returns>
        public static string GetIdCardSex(string strIdcard)
        {
            string sex = "男";
            if (strIdcard.Length == 15)
            {
                if (Convert.ToInt32(strIdcard.Substring(14, 1)) % 2 == 1)
                {
                    sex = "男";
            ...
            {
                if (Convert.ToInt32(strIdcard.Substring(16, 1)) % 2 == 1)
                {
                    sex = "男";
        ...
        {
            return ToDateTime(obj.ToString(), defValue);
        }
        ...
        /// <param name="Length"></param>
        /// <returns></returns>
        public static int NextNumber(int Length)
        {
            return Next(10, Length);
        }
        /// <summary>
        /// 生成随机数字
        /// </summary>
        ...
        public static string GetMapPath(string strPath)
        {
            ...
            return System.IO.Path.Combine(root, strPath);
        }
        #endregion
        ...
        public static void DeleteContentPic(string content, string startstr)
        {
            ...
            foreach (Match math in m)
            {
                ...
                if (imgUrl.IndexOf("http", StringComparison.OrdinalIgnoreCase) == 0) continue;
        ...
        /// <returns></returns>
        public static string ReplaceSpecilFileName(string name)
        {
            string str = string.Empty;
        ...
        /// <returns></returns>
        public static string HttpSSLGet(string url)
        {
        ...
        /// <param name="url"></param>
        /// <returns></returns>
        public static T HttpSSLGet<T>(string url)
        {
            string result = HttpSSLGet(url);
        ...
        /// <param name="param">POST的数据</param>
        /// <returns></returns>
        public static string HttpsPost(string url, string param, string contentType = "application/x-www-form-urlencoded")
        {
            //处理HttpWebRequest访问https有安全证书的问题（ 请求被中止: 未能创建 SSL/TLS 安全通道。）
            ServicePointManager.ServerCertificateValidationCallback += (s, cert, chain, sslPolicyErrors) => true;
            ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12 | SecurityProtocolType.Tls11;
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetWxUrlSuffix()
        {
        ...
        /// <returns></returns>
        public static DateTime ConvertToDateTimeFromInt(long timestamp)
        {
            System.DateTime startTime = TimeZone.CurrentTimeZone.ToLocalTime(new System.DateTime(1970, 1, 1));
            if (timestamp > 0)
            {
        ...
        /// <param name="timestamp"></param>
        /// <returns></returns>
        public static DateTime ConvertToDateTimeFrom13Int(long timestamp)
        {
            System.DateTime startTime = TimeZone.CurrentTimeZone.ToLocalTime(new System.DateTime(1970, 1, 1));
            if (timestamp > 0)
            {
        ...
        #region 获取时间差string GetTime
        /// <summary>
        /// 获取时间差
        /// </summary>
        /// <param name="getTime">数据库时间</param>
        /// <returns>时间差</returns>
        public static string GetMsgTime(DateTime time1)
        {
            var time = DateTime.Now - time1;
            if (time.TotalDays > 2)
            {
                return time1.ToString("M月d日 HH:mm");
            }
            //if (time.TotalDays > 1 && time.TotalDays < 2)
            if (DateTime.Now.Day - time1.Day == 1)
            {
                return "昨天 " + time1.ToString("HH:mm");
            }
            if (DateTime.Now.Day == time1.Day)
            {
        ...
        /// <param name="getTime">数据库时间</param>
        /// <returns>时间差</returns>
        public static string GetMsgDate(DateTime time1)
        {
            var time = DateTime.Now - time1;
            if (time.TotalDays > 2)
            {
                return time1.ToString("M月d日");
            }
            //if (time.TotalDays > 1 && time.TotalDays < 2)
            if ((DateTime.Now.Day - time1.Day) == 1)
            {
        ...
        /// <param name="getTime">数据库时间</param>
        /// <returns>时间差</returns>
        public static string GetTime(DateTime time1)
        {
            var time = DateTime.Now - time1;
        ...
        public static int NewRandom(int maxNum)
        {
        ...
        /// <param name="Length">生成长度</param>
        /// <param name="Sleep">是否要在生成前将当前线程阻止以避免重复</param>
        public static string Str_char(int num)
        {
            ...
            if (num > n)
            {
                var n1 = (num / n);
                var n2 = (num % n);
                result = result.PadLeft(n1, 'A');
                result += Pattern[n2 - 1].ToString();
            }
            else
            {
                result = Pattern[num - 1].ToString();
            }
        ...
        /// <returns></returns>
        public static string TrimZero(decimal amount)
        {
            string param = amount.ToString();
        ...
        /// <param name="o"></param>
        /// <returns></returns>
        public static uint ObjToUInt(object obj)
        {
            if (isNumber(obj))
            {
